...
- This reads the config properties from the zola.toml file by default, to override this use the `--config your-config.toml` option before the `build` command. 

**Serving**: `zola serve`

- Recommended to use an [ephemeral port](https://en.wikipedia.org/wiki/Ephemeral_port).

Detailed documentation for creating a site like this of your own using zola can be found [here](https://www.getzola.org/documentation/getting-started/overview/).
